...
void loadEEpromSettings() {
  ...
  if (eepromBuffer.drive_by_rpm >
    ...
    eepromBuffer.drive_by_rpm = 0;
  }
  if (eepromBuffer.maximum_rpm == 0 || eepromBuffer.maximum_rpm > 250) {
    eepromBuffer.maximum_rpm = 50; // 10000 rpm default
  }
  if (eepromBuffer.minimum_rpm == 0 || eepromBuffer.minimum_rpm > 250) {
    eepromBuffer.minimum_rpm = 5; // 1000 rpm default
  }
  if (eepromBuffer.brake_on_zero_throttle >
      9) { // byte 13 held a firmware name character before eeprom version 4
    eepromBuffer.brake_on_zero_throttle = 0;
  }
  ...
  //  than 1.90 or 10-42 with 1.90 or above
  if (eepromBuffer.advance_level > 42 ||
      (eepromBuffer.advance_level < 10 && eepromBuffer.advance_level > 3)) {
    temp_advance = 16;
  }
  if (eepromBuffer.advance_level <
      4) { // old format needs to be converted to 0-32 range
    temp_advance = (eepromBuffer.advance_level << 3);
    eepromBuffer.advance_level = temp_advance + 10;
  }
  if (eepromBuffer.advance_level < 43 &&
      eepromBuffer.advance_level > 9) { // new format subtract 10 from advance
  ...
  if (eepromBuffer.eeprom_version >
    ...
    if (eepromBuffer.driving_brake_strength < 10) {
      dead_time_override =
          deadTime + (150 - (eepromBuffer.driving_brake_strength * 10));
...
__RAMFUNC void tenKhzRoutine() { // 20khz as of 2.00 to be renamed
...
#define DUTY_LIMIT_99 1970
    ...
    if ((armed && running) && input > 47) {
      ...
      if (running && input < 47) { // brake on zero throttle behavior while
        ...
        if (eepromBuffer.brake_on_zero_throttle ==
            2) {             // motor brake on 0 throttle
          temp_comp_pwm = 1; // tracks rpm until stopped
          zero_throttle_brake_active = 1;
        }
        if ((eepromBuffer.brake_on_zero_throttle > 2) &&
            (eepromBuffer.brake_on_zero_throttle <
             10)) { // brake on 0 throttle after 2 + x seconds
          if (zero_throttle_brake_active == 0) {
            brake_countdown = eepromBuffer.brake_on_zero_throttle -
                              2; // brake countdown decremented in 20khz routine
            tenkhzcounter = LOOP_FREQUENCY_HZ;
          }
          if ((brake_countdown == 0) && (zero_throttle_brake_active == 1)) {
            zero_crosses =
                0;       // after countdown forces the brake on stop behavior
            running = 0; // stops tracking rpm
          }
          zero_throttle_brake_active = 1;
        }
        adjusted_duty_cycle = ((duty_cycle * tim1_arr) / 2000);
      } else { // input less than 47 and not running, normal brake on stop
               // behavior
        if (prop_brake_active) {
          adjusted_duty_cycle =
              tim1_arr - ((prop_brake_duty_cycle * tim1_arr) / 2000);
        } else {
          if ((eepromBuffer.brake_on_stop == 2) &&
              armed) { // require arming for active brake
            comStep(2);
            adjusted_duty_cycle =
                DEAD_TIME +
                ((eepromBuffer.active_brake_power * tim1_arr) / 2000) * 10;
          } else {
            adjusted_duty_cycle = ((duty_cycle * tim1_arr) / 2000);
          }
        }
...
void zcfoundroutine() { // only used in polling mode, blocking routine.
  ...
  if (eepromBuffer.no_polling_start) {
    // #ifdef NO_POLLING_START     // changes to interrupt mode after 2 zero
    // crosses, does not re-enter
    if (zero_crosses > 2) {
      old_routine = 0;
...
// const char filename[30] __attribute__((section(".__device_info_start"))) =
// FILE_NAME;

int main(void) {
  eeprom_address =
      (uint32_t)(&_eeprom_start_addr) + (uint32_t)(&_flash_rw_offset);
  ...
  if (FIRMWARE_VERSION_MAJOR != eepromBuffer.version.major ||
      FIRMWARE_VERSION_MINOR != eepromBuffer.version.minor ||
  ...
  // minimum_duty_cycle = (minStartupDuty/ 2 + DEAD_TIME/3) * TIMER1_MAX_ARR /
  // 2000 ; stall_protect_minimum_duty = minimum_duty_cycle+10;
...
// #endif
  ...
  while (1) {
    ...
    //         if (dshot_telemetry && (commutation_interval >
    //         DSHOT_PRIORITY_THRESHOLD)) {
    ...
    if (PROCESS_ADC_FLAG ==
        1) { // for adc and telemetry set adc counter at 1khz loop rate
#if defined(STMICRO)
      ADC_DMA_Callback();
      ...
      if (eepromBuffer.low_voltage_cut_off == 1) {
        if (battery_voltage < (cell_count * low_cell_volt_cutoff)) {
          ...
          if (!LOW_VOLTAGE_CUTOFF) { // if set low cutoff has happened, require
      ...
      if (low_voltage_count >
          (10000 - (stepper_sine *
                    9900))) { // 10 second wait before cut-off for low voltage
        LOW_VOLTAGE_CUTOFF = 1;
        input = 0;
        allOff();
        maskPhaseInterrupts();
    ...
    if (stepper_sine == 0) {
      ...
      if (INTERVAL_TIMER_COUNT > 45000) {
        zero_throttle_brake_active =
            0; // reset zero throttle brake on back emf timeout (rotation stop)
        if (running == 1) {
          temp_comp_pwm = eepromBuffer.comp_pwm;
