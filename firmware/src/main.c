...
void loadEEpromSettings() {
  ...
  //    eepromBuffer.absolute_voltage_cutoff = 10;  // voltage level 1 to 100 in
  //    0.5v increments eepromBuffer.current_P = 100; // 0-255
  ...
  // RPM control settings (eeprom positions 13-15)
  drive_by_rpm = eepromBuffer.drive_by_rpm;
  if (eepromBuffer.min_rpm_percent > 0 && eepromBuffer.min_rpm_percent <= 100) {
    MINIMUM_RPM_SPEED_CONTROL =
        (motor_kv * 1000 / 100) * eepromBuffer.min_rpm_percent / 100;
    if (MINIMUM_RPM_SPEED_CONTROL < 100)
      MINIMUM_RPM_SPEED_CONTROL = 100;
  }
  if (eepromBuffer.max_rpm_percent > 0 && eepromBuffer.max_rpm_percent <= 100) {
    MAXIMUM_RPM_SPEED_CONTROL =
        (motor_kv * 1000 / 100) * eepromBuffer.max_rpm_percent / 100;
    if (MAXIMUM_RPM_SPEED_CONTROL < 1000)
      MAXIMUM_RPM_SPEED_CONTROL = 1000;
  }
...
// #endif
  ...
  while (1) {
    ...
    if (stepper_sine == 0) {
      ...
      // Improved filtering for low KV motors
      if (motor_kv < 900 && filter_level < low_kv_filter_level) {
