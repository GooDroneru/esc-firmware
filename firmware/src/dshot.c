...
__RAMFUNC void computeDshotDMA() {
  ...
  if ((dshot_frametime > dshot_frametime_low) &&
    ...
    signaltimeout = 0;
    for (int i = 0; i < 16; i++) {
      // note that dma_buffer[] is uint32_t, we cast the difference to uint16_t
      // to handle timer wrap correctly
    ...
    if (calcCRC == checkCRC) {
      ...
      if (programming_mode > 0) {
        ...
        if (programming_mode == 3) {
          if (tocheck == 37) { // commit new values to eeprom. must use save
                               // settings to make permanent.
      ...
      if ((dshotcommand > 0) && (running == 0) && armed) {
        ...
        if (command_count >= 6) {
          ...
          case 36:
            programming_mode = 1;
            //          armed = 0;           // disarm when entering programming
            //          mode
            break;
          }
...
__RAMFUNC void make_dshot_package(uint16_t com_time) {
  ...
  if (dshot_extended_telemetry) {
    // Only send extended telemetry if last frame wasn't extended. This ensures
    // eRPM interleaving.
  ...
  if (extended_frame_to_send > 0) {
    ...
    shift_amount =
        0; // Extended frames use fixed format, no range extension needed
