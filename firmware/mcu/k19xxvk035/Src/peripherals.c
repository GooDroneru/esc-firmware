...
void PWM_TIM1_Init(void) // PWM
{
  ...
  GPIOA->DENSET |= GPIO_DENSET_PIN8_Msk | GPIO_DENSET_PIN9_Msk |
                   GPIO_DENSET_PIN10_Msk | GPIO_DENSET_PIN11_Msk |
                   GPIO_DENSET_PIN12_Msk | GPIO_DENSET_PIN13_Msk;
  ...
  PWM0->ETSEL_bit.SOCASEL =
      PWM_ETSEL_SOCASEL_CTREqCMPA_OnDown; // ADC trigger при counter = CMPA при
                                          // счёте вниз (конец активного
                                          // периода)
...
void disableComTimerInt() { COM_TIMER->CTRL_bit.INTEN = 0; }

void enableComTimerInt() { COM_TIMER->CTRL_bit.INTEN = 1; }
...
void reloadWatchDogCounter() { WDT->INTCLR = 1; }
...
uint32_t rawBuffer[64] __attribute__((section(".ram_section")))
__attribute__((aligned(4))) = {0};
static volatile DMA_CtrlData_TypeDef DMA_CONFIGDATA
...
void UN_TIM2_Init(void) {
  ...
  DMA_CONFIGDATA.PRM_DATA.CH[12].SRC_DATA_END_PTR =
      (uint32_t)&(gcr[buffer_padding + 21]); // Адрес конца источника данных
                                             // (последний заполненный элемент)
  DMA_CONFIGDATA.PRM_DATA.CH[12].CHANNEL_CFG_bit.SRC_SIZE =
      DMA_CHANNEL_CFG_SRC_SIZE_Word; // Разрядность данных источника
  DMA_CONFIGDATA.PRM_DATA.CH[12].CHANNEL_CFG_bit.SRC_INC =
      DMA_CHANNEL_CFG_SRC_INC_Word; // Инкрементируем источник по словам
  /* приемник */
  DMA_CONFIGDATA.PRM_DATA.CH[12].DST_DATA_END_PTR =
      (uint32_t)(&IC_TIMER_REGISTER->CMP); // Адрес конца данных приемника
  DMA_CONFIGDATA.PRM_DATA.CH[12].CHANNEL_CFG_bit.DST_SIZE =
      DMA_CHANNEL_CFG_SRC_SIZE_Word; // Разрядность данных приемника
  DMA_CONFIGDATA.PRM_DATA.CH[12].CHANNEL_CFG_bit.DST_INC =
      DMA_CHANNEL_CFG_DST_INC_None; // Инкрементируем на байт
  /* общее */
  ...
  DMA_CONFIGDATA.PRM_DATA.CH[12].CHANNEL_CFG_bit.N_MINUS_1 =
      22 - 1; // Общее количество передач DMA (22 слов:
              // gcr[buffer_padding]..gcr[buffer_padding+21])
  DMA_CONFIGDATA.PRM_DATA.CH[12].CHANNEL_CFG_bit.CYCLE_CTRL =
      DMA_CHANNEL_CFG_CYCLE_CTRL_Basic; // Задание типа цикла DMA

  DMA_CONFIGDATA.PRM_DATA.CH[8].SRC_DATA_END_PTR =
      (uint32_t)(&TMR3->VALUE); // Адрес источника данных
  DMA_CONFIGDATA.PRM_DATA.CH[8].CHANNEL_CFG_bit.SRC_SIZE =
      DMA_CHANNEL_CFG_SRC_SIZE_Word; // Разрядность данных источника
  DMA_CONFIGDATA.PRM_DATA.CH[8].CHANNEL_CFG_bit.SRC_INC =
      DMA_CHANNEL_CFG_SRC_INC_None; // Не инкрементируем
  /* приемник */
  DMA_CONFIGDATA.PRM_DATA.CH[8].DST_DATA_END_PTR =
      (uint32_t)&(rawBuffer[32 - 1]); // Адрес конца данных приемника
  DMA_CONFIGDATA.PRM_DATA.CH[8].CHANNEL_CFG_bit.DST_SIZE =
      DMA_CHANNEL_CFG_SRC_SIZE_Word; // Разрядность данных приемника
  DMA_CONFIGDATA.PRM_DATA.CH[8].CHANNEL_CFG_bit.DST_INC =
      DMA_CHANNEL_CFG_DST_INC_Word; // Инкрементируем на байт
  /* общее */
  DMA_CONFIGDATA.PRM_DATA.CH[8].CHANNEL_CFG_bit.R_POWER =
      0x0; // Количество передач до переарбитрации
  DMA_CONFIGDATA.PRM_DATA.CH[8].CHANNEL_CFG_bit.N_MINUS_1 =
      32 - 1; // Общее количество передач DMA
  DMA_CONFIGDATA.PRM_DATA.CH[8].CHANNEL_CFG_bit.CYCLE_CTRL =
      DMA_CHANNEL_CFG_CYCLE_CTRL_Basic; // Задание типа цикла DMA
...
__RAMFUNC void updateDma() {
  ...
  DMA_CONFIGDATA.PRM_DATA.CH[8].CHANNEL_CFG_bit.R_POWER =
      0x0; // Количество передач до переарбитрации
  DMA_CONFIGDATA.PRM_DATA.CH[8].CHANNEL_CFG_bit.N_MINUS_1 =
      buffersize - 1; // Общее количество передач DMA
  DMA_CONFIGDATA.PRM_DATA.CH[8].CHANNEL_CFG_bit.CYCLE_CTRL =
      DMA_CHANNEL_CFG_CYCLE_CTRL_Basic; // Задание типа цикла DMA
  DMA_CONFIGDATA.PRM_DATA.CH[8].DST_DATA_END_PTR =
      (uint32_t)&(rawBuffer[buffersize - 1]);
  DMA->ENSET_bit.CH8 = 1;
}
