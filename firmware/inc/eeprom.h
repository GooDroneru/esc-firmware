...
typedef union EEprom_u {
  struct {
    ...
    uint8_t
        absolute_voltage_cutoff; // 8  voltage level 1 to 100 in 0.5v increments
    ...
    union {
      ...
      struct {
        uint8_t drive_by_rpm; // 13 — 0/1, режим управления по RPM
        uint8_t min_rpm_percent; // 14 — 0-100, % от max RPM для min throttle
        uint8_t max_rpm_percent; // 15 — 0-100, % от max RPM для max throttle
        uint8_t reserved_eeprom_3_3; // 16 — зарезервировано
      };
    };
